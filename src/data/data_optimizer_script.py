    ...
    def convert_series(self, array_of_series):
        ...
        for series in array_of_series:
            print(
                ...
                series.name,
                " size (MB):",
                round(series.memory_usage(deep=True) * 1e-6, 2),
            )
...
def optimize_chunk(chunk, i, original_data_size, new_data_size):
    ...
    [sample_data["timestamp"]] = timestamp_converter.convert_series(
    ...
    [
        sample_data["title"],
        sample_data["world"],
        sample_data["event_code"],
        sample_data["event_count"],
        sample_data["installation_id"],
    ] = category_converter.convert_series(
        [
            sample_data["title"],
            sample_data["world"],
            sample_data["event_code"],
    ...
    print(
        "The initial space on disk was (MB): ",
        round(initial_memory_usage, 2),
        " and the final space on disk is (MB): ",
...
for chunk in pd.read_csv("../../data/raw/train.csv", chunksize=chunksize):
    i += 1
    print(i)
    original_data_size, new_data_size, sample_data = optimize_chunk(
        chunk, i, original_data_size, new_data_size
    )
    print(
        "The total original data size was: "
        + str(round(original_data_size, 2))
        + " and the new total data size is: "
        + str(round(new_data_size, 2))
    )
    if i == 1:
